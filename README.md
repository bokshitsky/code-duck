...
`--project-root` — модулем считается любой каталог с `src/main/java` на любой
глубине (каталоги внутри `target` пропускаются). Имя вложенного модуля — путь
относительно корня, например `hh-fixture/server`.
