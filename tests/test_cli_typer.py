...
def test_java_analysis_passes_modules_to_exporter(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_java_analysis_discovers_modules_with_maven_all(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    project_root = tmp_path
    output = project_root / 'model.duckdb'
    calls: list[tuple[Path, Sequence[str], Path, str, bool]] = []
...
def test_java_analysis_fails_when_output_exists(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    project_root = tmp_path
    output = project_root / 'model.duckdb'
...
def test_java_analysis_passes_force_flag_to_exporter(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    project_root = tmp_path
    output = project_root / 'model.duckdb'
    calls: list[tuple[Path, Sequence[str], Path, str, bool]] = []
...
def test_python_analysis_passes_packages_to_exporter(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
