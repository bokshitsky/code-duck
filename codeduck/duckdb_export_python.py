...
COMMENT_STATEMENTS = (
    ...
    "COMMENT ON COLUMN python_packages.path IS 'Путь пакета в posix относительно родителя корневого каталога, например myapp/sub'",
    "COMMENT ON TABLE python_modules IS 'Модули (.py-файлы) целевых пакетов; __init__.py представляет сам пакет'",
    "COMMENT ON COLUMN python_modules.module_id IS 'Суррогатный идентификатор модуля; на него ссылаются остальные таблицы'",
    "COMMENT ON COLUMN python_modules.package_id IS 'Пакет-владелец (каталог файла); ссылка на python_packages.package_id'",
    ...
    "COMMENT ON COLUMN python_modules.path IS 'Путь файла в posix относительно родителя корневого каталога, например myapp/sub/mod.py'",
    ...
    "COMMENT ON COLUMN python_imports.imported_name IS 'Полное импортируемое имя; для from — module.name, для относительных начинается с точки'",
    ...
    "COMMENT ON COLUMN python_classes.bases IS 'Базовые классы через запятую как в исходнике (сырой текст); пустая строка при отсутствии'",
    "COMMENT ON COLUMN python_classes.decorators IS 'Декораторы класса через запятую без ведущего @; пустая строка при отсутствии'",
    ...
    "COMMENT ON COLUMN python_functions.class_id IS 'Класс-владелец для метода; ссылка на python_classes.class_id; NULL для функции уровня модуля'",
    "COMMENT ON COLUMN python_functions.name IS 'Простое имя функции/метода'",
    "COMMENT ON COLUMN python_functions.qualified_name IS 'Имя с учётом вложенности: module.Class.method или module.func.nested'",
    "COMMENT ON COLUMN python_functions.params IS 'Параметры через запятую как в исходнике (с аннотациями и значениями по умолчанию, включая *args/**kwargs)'",
    "COMMENT ON COLUMN python_functions.returns IS 'Аннотация возвращаемого значения (текст после ->); NULL, если не указана'",
    "COMMENT ON COLUMN python_functions.decorators IS 'Декораторы через запятую без ведущего @; пустая строка при отсутствии'",
    ...
    "COMMENT ON COLUMN python_calls.caller_function_id IS 'Обрамляющая функция/метод; ссылка на python_functions.function_id; NULL для кода уровня модуля'",
    "COMMENT ON COLUMN python_calls.callee_expr IS 'Сырое выражение вызываемого (текст перед скобками), например self.repo.save'",
    "COMMENT ON COLUMN python_calls.callee_name IS 'Последний сегмент вызова (имя функции/атрибута); NULL, если не выделяется'",
    "COMMENT ON COLUMN python_calls.resolved_target IS 'Разрешённая цель (FQN) по импортам/локальным определениям/self; NULL, если однозначно не определено'",
...
def write_database(
    ...
    with log_duration("Подготовка строк для вставки"):
        ...
        for module_id, module in enumerate(module_models, start=1):
            module_rows.append(
                [module_id, package_id_by_name[module.package_name], 1, module.name, module.path]
            )
            for import_model in module.imports:
                import_id += 1
                import_rows.append(
                    [import_id, module_id, import_model.kind, import_model.imported_name, import_model.alias]
                )
            ...
            for call_model in module.calls:
                call_id += 1
                caller_id = (
                    function_id_by_qname.get(call_model.caller)
                    if call_model.caller is not None
                    else None
                )
                call_rows.append(
                    [
