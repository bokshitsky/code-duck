...
class PythonAnalyzer:
    ...
    def analyze(self) -> tuple[list[PackageModel], list[ModuleModel]]:
        ...
        with log_duration('Чтение и парсинг исходников'):
            for package_dir in self.package_dirs:
                ...
                modules.extend(
                    self._read_module(path, root_parent, packages) for path in sorted(package_dir.rglob('*.py'))
                )
    ...
    def _visit(
        ...
        if node_type == 'function_definition':
            self._visit_function(node, source, prefix, class_qname, parent_is_class, module, raw_calls)
            return
        if node_type == 'call':
    ...
    @staticmethod
    def _dotted_parts(node: Node, source: bytes) -> tuple[str, ...] | None:
        """
        Разбирает цепочку identifier/attribute в список сегментов, иначе None.

        Returns:
            Кортеж сегментов или ``None``, если узел не разбирается.
    ...
    def _alias_map(imports: Sequence[ImportModel]) -> dict[str, str]:
        ...
        Returns:
            Отображение связанного имени на импортируемую цель.
    ...
    def _defs_map(module: ModuleModel) -> dict[str, str]:
        ...
        Returns:
            Отображение простого имени на qualified_name.
    ...
    def _decorators(self, node: Node, source: bytes) -> tuple[str, ...]:
        ...
        if parent is None or parent.type != 'decorated_definition':
            return ()
        decorators = [
            self._text(child, source).removeprefix('@').strip()
