...
SCHEMA_STATEMENTS = (
    ...
    'CREATE TABLE java_modules (  module_name TEXT PRIMARY KEY,  repo_id INTEGER NOT NULL REFERENCES repos(repo_id))',
...
COMMENT_STATEMENTS = (
    ...
    "COMMENT ON TABLE java_classes IS 'Верхнеуровневые Java-типы (class/interface/enum/record/annotation) целевых модулей'",  # noqa: E501
    "COMMENT ON COLUMN java_classes.module_name IS 'Модуль, в котором объявлен тип; ссылка на java_modules.module_name'",  # noqa: E501
    "COMMENT ON COLUMN java_classes.source_type IS 'Каталог исходников: prod (src/main/java) или test (src/test/java)'",
    "COMMENT ON COLUMN java_classes.class_id IS 'Суррогатный идентификатор класса; на него ссылаются остальные таблицы'",  # noqa: E501
    ...
    "COMMENT ON COLUMN java_methods.class_fqn IS 'Полное имя класса-владельца метода; денормализованная копия java_classes.fqn'",  # noqa: E501
    "COMMENT ON COLUMN java_methods.method_name IS 'Имя метода; для конструктора совпадает с именем класса'",
    "COMMENT ON COLUMN java_methods.params IS 'Типы параметров через запятую в порядке объявления; varargs как Тип...; пустая строка при отсутствии параметров'",  # noqa: E501
    "COMMENT ON COLUMN java_methods.return_type IS 'Тип возвращаемого значения (void для процедур); NULL для конструктора'",  # noqa: E501
    "COMMENT ON TABLE java_class_dependencies IS 'Связи класса с другими типами по их FQN: использование, наследование и аннотации'",  # noqa: E501
    "COMMENT ON COLUMN java_class_dependencies.from_class_id IS 'Класс-источник связи; ссылка на java_classes.class_id'",  # noqa: E501
    "COMMENT ON COLUMN java_class_dependencies.from_fqn IS 'Полное имя класса-источника связи; денормализованная копия java_classes.fqn'",  # noqa: E501
    "COMMENT ON COLUMN java_class_dependencies.relation_type IS 'Тип связи: annotated_by (аннотация класса), extends (наследование), implements (реализация интерфейса), uses (использование типа в коде)'",  # noqa: E501
    "COMMENT ON COLUMN java_class_dependencies.to_fqn IS 'Полное имя типа, с которым связан класс (аннотация, суперкласс, интерфейс или используемый тип)'",  # noqa: E501
    ...
    "COMMENT ON COLUMN java_method_annotations.annotation_fqn IS 'Имя аннотации, разрешённое до FQN там, где это возможно'",  # noqa: E501
)
...
def write_database(
    ...
    with log_duration('Подготовка строк для вставки'):
        ...
        method_rows = []
        method_annotation_rows: list[list[object]] = []
        ...
        for class_id, model in enumerate(class_models, start=1):
            for method in model.methods:
                ...
                method_annotation_rows.extend([method_id, annotation_fqn] for annotation_fqn in method.annotations)
            dependency_rows.extend(
                [class_id, model.fqn, 'annotated_by', annotation_fqn] for annotation_fqn in model.annotations
            )
            for super_fqn, relation_type in model.supertypes:
                dependency_rows.append([class_id, model.fqn, relation_type, super_fqn])
            dependency_rows.extend(
                [class_id, model.fqn, 'uses', dependency_fqn] for dependency_fqn in model.dependencies
            )
    ...
    with log_duration('Вставка данных в DuckDB'):
        ...
        insert('java_method_annotations', [('method_id', integer), ('annotation_fqn', text)], method_annotation_rows)
        insert(
            'java_class_dependencies',
            [
